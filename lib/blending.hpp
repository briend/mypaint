...
inline float mod_arith(float a, float b)
{
    float ret = a - b * floor (a / b);
    return ret;
}

// Returns the smallest angular difference
inline float smallest_angular_difference(float angleA, float angleB)
{
    float a;
    a = angleB - angleA;
...
//                  _slope += src[o+c];
//                }
                slopes[1] += -2.0 * src[o+MYPAINT_NUM_CHANS-2];
                //printf("%f \n", src[o+MYPAINT_NUM_CHANS-2]);
              }
...
//                  _slope += src[o+c];
//                }
                slopes[0] += -2.0 * src[o+MYPAINT_NUM_CHANS-2];
              }
              // West
              {
                int o = i - MYPAINT_NUM_CHANS * p;
...
//                }
            ...
            float degrees = atan(slope * direction / 180.0);
